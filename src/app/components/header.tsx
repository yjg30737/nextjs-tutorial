...
export function Header({ header }: { header: string }) {
    ...
    const renderThemeChanger = () => {
        ...
        if(currentTheme ==="dark"){
          return (
            <button className="w-10 h-10" role="button" onClick={() => setTheme('light')}>
          ...
          return (
            <button className="w-10 h-10" role="button" onClick={() => setTheme('dark')}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
    ...
    return <header className="p-4 shadow-md">
        <div className="container flex mx-auto items-center justify-between">
            <h1 className="mx-auto text-2xl font-bold ">{header}</h1>
