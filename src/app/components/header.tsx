...
export function Header({ header }: { header: string }) {
    ...
    //       return (
    //         <button className={`hidden md:flex ${ isNavVisible ? 'block' : 'hidden' }`} role="button" onClick={() => setTheme('light')}>
    ...
    //     else {
    //       return (
    //         <button className={`hidden md:flex ${ isNavVisible ? 'block' : 'hidden' }`} role="button" onClick={() => setTheme('dark')}>
    //             <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
    ...
    //     <div>
        ...
        //             </div>
        //         </nav>
        return <nav className="flex items-center justify-between flex-wrap bg-gray-800 p-6">
            <div className="flex items-center flex-shrink-0 text-white mr-6">
            <span className="font-semibold text-xl tracking-tight">{header}</span>
            </div>
            <div className="block lg:hidden">
            <button className="flex items-center px-3 py-2 border rounded text-gray-200 border-gray-400 hover:text-white hover:border-white" onClick={toggleNav}>
            ...
            <div className="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
                ...
                <div className="text-sm">
                    ...
                    <a href="/project" className={`${isNavVisible ? 'block' : 'hidden'} md:flex mt-4 lg:inline-block lg:mt-0 text-gray-200 hover:text-white mr-4`}>
                        Projects
                    </a>
                    <a href="/about" className={`${isNavVisible ? 'block' : 'hidden'} md:flex mt-4 lg:inline-block lg:mt-0 text-gray-200 hover:text-white`}>
                        About
                    </a>
                </div>
            </div>
